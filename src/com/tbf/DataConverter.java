...
public class DataConverter {
	...
	/**
	 * This method will open data/Persons.dat file, take in the data, change it to
	 * xml, then output it to data/Persons.xml whenever it's called
	 */
	public static void printPersonToXML() {
		try {
			...
			for (int i = 0; i < size; i++) {
				...
				if (broker[0].contains("E") || broker[0].contains("J")) {
					inputUsers[i] = new User(token[0], p1, names[1].replaceAll("\\s+", ""), names[0], broker[0],
							broker[1], emails);
				} else {
			...
			for (int i = 0; i < size; i++) {
				result = xstream.toXML(inputUsers);
			}
	...
	/**
	 * This method will open data/Assets.dat file, take in the data, change it to
	 * xml, then output it to data/Assets.xml whenever it's called
	 */
	public static void printAssetToXML() {
		try {
			...
			for (int i = 0; i < size; i++) {
				...
				if (temp.contains(";D;")) {
					...
					PrivateInvestments tempB = new PrivateInvestments(token[0], token[2], Double.parseDouble(token[3]),
							Double.parseDouble(token[4]) / 100, Double.parseDouble(token[5]),
